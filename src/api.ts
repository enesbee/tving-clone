...
export async function getThemes() {
  const response = await fetch(
    `${BASE_URL}/operator/themes?pageNo=1&pageSize=2&contentOutputLimit=20&pocCode=${POC_CODE}&themeType=T&exposeMainYn=Y&screenCode=${SCREEN_CODE}&osCode=${OS_CODE}&teleCode=${TELE_CODE}&apiKey=${API_KEY}`
  );
  return await response.json();
}
